...
public:
	...
	int maxwidth = 375;
	int maxheight = 500;
	D3DXCOLOR Background = D3DXCOLOR(0.204, .0, .155, .5);
	...
	void Initialize(std::wstring _title) {
		this->title = _title;
	}
	...
	void Render() {
		...
		//DrawBox(this->positionx, this->positiony, this->maxwidth, heightm, 1, Background.r, Background.g, Background.b, Background.a, true); //background

		//DrawWString(this->title, 20, this->positionx + 10, this->positiony + spacing - 10, this->Text_Titles.r, this->Text_Titles.g, this->Text_Titles.b);

		if (!this->show)
		{
			//DrawWString(L"[Insert]: Show/Hide Menu", 20, this->positionx + 10, this->positiony + spacing * 2 - 10, this->Text_Items.r, this->Text_Items.g, this->Text_Items.b);
		}
		else {
			for (int tabindex = 0; tabindex < Tabs.size(); tabindex++) {
				...
				DrawLine(x1, y1, x2, y1, thicc, Borders.r, Borders.g, Borders.g);
				DrawLine(x2, y1, x2, y2, thicc, Borders.r, Borders.g, Borders.g);
				DrawLine(x2, y2, x1, y2, thicc, Borders.r, Borders.g, Borders.g);
				DrawLine(x1, y2, x1, y1, thicc, Borders.r, Borders.g, Borders.g);

				DrawWString(Tabs[tabindex].label, 15, x1 + 5, y1, this->Text_Titles.r, this->Text_Titles.g, this->Text_Titles.b);
			}


			for (int itemindex = 0; itemindex < this->Tabs[this->SelectedMenu].Items.size(); itemindex++) {
				DrawWString(this->Tabs[this->SelectedMenu].Items[itemindex].name,
				...
				if (this->Tabs[this->SelectedMenu].Items[itemindex].setting != nullptr) {
					DrawWString(std::to_wstring(*this->Tabs[this->SelectedMenu].Items[itemindex].setting),
			...
			DrawWString(L"->", 20, this->positionx + 5, this->positiony + (this->SelectedItem * (spacing)+(this->spacing * 4)), 1, 1, 0);
