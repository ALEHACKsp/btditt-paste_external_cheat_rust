...
void DrawWString(std::wstring str, float fontSize, float x, float y, float r, float g, float b, float a)
{
	RECT re;
	GetClientRect(overlayWindow, &re);
	FLOAT dpix, dpiy;
	dpix = static_cast<float>(re.right - re.left);
	dpiy = static_cast<float>(re.bottom - re.top);
	HRESULT res = w_factory->CreateTextLayout(str.c_str(), str.length() + 1, w_format, dpix, dpiy, &w_layout);
	if (SUCCEEDED(res))
	{
		DWRITE_TEXT_RANGE range = { 0, str.length() };
		w_layout->SetFontSize(fontSize, range);
...
void DirectOverlaySetup(DirectOverlayCallback callback) {
	...
	MyOverlayCreateThread(OverlayThread, NULL, NULL);
}
...
void DirectOverlaySetup(DirectOverlayCallback callback, HWND _targetWindow) {
	...
	MyOverlayCreateThread(OverlayThread, _targetWindow, NULL);
}
