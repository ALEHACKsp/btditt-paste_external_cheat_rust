...
0x6E17, 0x7E36, 0x4E55, 0x5E74, 0x2E93, 0x3EB2, 0x0ED1, 0x1EF0
   ...
   unsigned short CRC(unsigned char* pcBlock, unsigned short len)
	...
	wchar_t* GetShortName(int* len){
		DWORD64 info = mex.Read<DWORD64>(Item + 0x18);
		pUncStr str = ((pUncStr)mex.Read<DWORD64>(info + 0x20));
		int leng = mex.Read<int>((uintptr_t)str + 0x10);
	...
	enum BoneList : int
{
	...
	displacement
		...
		switch(GetHeldItem().GetNameHash()){
		case 0:
			return 0.f;
	...
	Vector3 GetVelocity() {
        DWORD64 PlayerModel = mex.Read<DWORD64>(Player + 0x490); //public PlayerModel playerModel;
        return mex.Read<Vector3>(PlayerModel + 0x1EC); //private Vector3 newVelocity; !!!
 
    }
	Vector3 Prediction(BasePlayer enemy,int bone){
		Vector3 bonePos = enemy.GetBonePosition(bone);
		float Dist = Math::Calc3D_Dist(enemy.GetBonePosition(spine3), bonePos);

		if (Dist > 0.001f) {
			float BulletTime = Dist / GetBulletSpeed();
			Vector3 vel = enemy.GetVelocity();
			Vector3 Predict = vel * BulletTime * 0.75f;
			bonePos += Predict; bonePos.y += (4.905f * BulletTime * BulletTime);
	...
	void SetVA(const Vector2& VA) {
        DWORD64 Input = mex.Read<DWORD64>(Player + 0x5D8);//public PlayerInput input;
        mex.Write<Vector2>(Input + 0x3C, VA);//private Vector3 bodyAngles;
    }

    void SetRA() {
        DWORD64 Input = mex.Read<DWORD64>(Player + 0x5D84);//public PlayerInput input;
        Vector2 RA = { 0.f, 0.f };
        mex.Write<Vector2>(Input + 0x64, RA);//public Vector3 recoilAngles;
    }

    Vector2 GetVA() {
        DWORD64 Input = mex.Read<DWORD64>(Player + 0x5D8);//public PlayerInput input;
        return mex.Read<Vector2>(Input + 0x3C);//private Vector3 bodyAngles;
    }

    Vector2 GetRA() {
        DWORD64 Input = mex.Read<DWORD64>(Player + 0x5D8);//public PlayerInput input;
        return mex.Read<Vector2>(Input + 0x64);//public Vector3 recoilAngles;
    }
